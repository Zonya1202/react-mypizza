...
const Search: React.FC = () => {
  ...
  const onClickClear = () => {
    ...
    setValue('');
    inputRef.current?.focus(); // ? означает если здесь inputRef.current будет null или undefined то код после знака не выполняй
  };

  const updateSearchValue = React.useCallback(
    // работает как useEffect только на функцию что бы она постоянно не перевызывалась
    debounce((str) => {
